...
动态代理可以使用Jdk方式也可以使用CGLIB，他们的区别，如下；
...
|调用方法|方法传入obj参数|fastclass|代理类型|被代理类限制|                                                                                                                                                                                    
|---|---|---|---|---|
|methodProxy.invokeSuper()|代理类实例,由callback方法传入|生成的代理类的fastclass|静态代理|被代理类必须是实现类|
|methodProxy.invoke()|被代理类实例, 可以自行创建多个并调用多次|被代理类的fastclass|动态代理|被代理类可以是接口|
